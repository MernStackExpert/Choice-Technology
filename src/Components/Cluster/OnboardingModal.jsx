  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        const updateRes = await axiosInstance.patch("/auth/user/profile", {
          onForm: true,
          firebaseUid: user?.uid,
        });
          ...
          >
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
              <div className="flex flex-col items-center mb-10">
                ...
                <p className="text-[10px] font-black text-cyan-500/50 uppercase mt-4 tracking-widest">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input
                    {...register("name")}
                    disabled
                    className="w-full bg-white/5 border border-white/5 rounded-2xl px-5 py-4 text-white/40 text-sm cursor-not-allowed"
                  />
                </div>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-cyan-500 uppercase ml-2">
                    National ID (NID)
                  </label>
                  <input
                    {...register("nid", {
                      required: "NID is required",
                      minLength: { value: 10, message: "Invalid NID format" },
                    })}
              ...
              <div className="pt-6">
                ...
                >
                  {loading ? (
                    ...
                    <>
                      Verify & Connect Node <CheckCircle size={20} />
                    </>
                  )}
                </button>
              </div>
