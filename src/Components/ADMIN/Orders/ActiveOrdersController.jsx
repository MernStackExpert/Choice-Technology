...
import { 
  Search, Loader2, Eye, Activity, 
  ChevronLeft, ChevronRight, Zap 
} from "lucide-react";
import Link from "next/link";
import toast from "react-hot-toast";

export default function ActiveOrdersController() {
  ...
  const fetchActiveOrders = useCallback(async () => {
    try {
      ...
      if (res.data.success) {
        const activeOnly = res.data.result.filter(order => order.status === "active");
        setOrders(activeOnly);
        setTotalPages(res.data.totalPages);
        setTotalActiveCount(activeOnly.length);
      }
    } catch (error) {
      toast.error("Failed to sync active nodes");
    } finally {
      setLoading(false);
    }
        ...
        <div className="space-y-4">
          ...
          <div>
            <h2 className="text-4xl font-black text-white uppercase tracking-tighter flex items-center gap-3">
              <Activity className="text-emerald-500" /> Node_Stream
            </h2>
            <p className="text-[10px] text-gray-500 font-bold uppercase tracking-[0.2em] mt-1">
              Operational synchronization in progress
            </p>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead>
              <tr className="border-b border-white/5 bg-white/[0.03]">
                ...
                <th className="p-7 text-[9px] font-black text-gray-500 uppercase tracking-widest">Financials</th>
                <th className="p-7 text-[9px] font-black text-gray-500 uppercase tracking-widest">Sync_Progress</th>
              ...
              {loading ? (
                ...
                <tr key={order._id} className="hover:bg-white/[0.05] transition-all group">
                  ...
                  <td className="p-7">
                    ...
                    <p className="text-[9px] text-gray-500 lowercase mt-1 font-medium italic">{order.userEmail}</p>
                  </td>
                  <td className="p-7">
                    <p className="text-[11px] text-white font-mono font-bold">${order.totalAmount}</p>
                    <div className="flex items-center gap-1.5 mt-1.5">
                      <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                      <p className="text-[9px] text-emerald-500 uppercase font-black tracking-tighter">Paid: ${order.paidAmount}</p>
                    </div>
                  </td>
                  <td className="p-7">
                    <div className="flex items-center gap-4">
                      <div className="flex-1 max-w-[120px] bg-white/10 h-1.5 rounded-full overflow-hidden">
                        <div 
                          className="bg-gradient-to-r from-cyan-600 to-cyan-400 h-full transition-all duration-1000 ease-out shadow-[0_0_10px_#22d3ee]" 
                          style={{ width: `${order.progress}%` }}
                        />
                      </div>
                      <span className="text-[10px] font-mono text-white font-bold">{order.progress}%</span>
                    </div>
                  </td>
      ...
      {!loading && orders.length > 0 && (
        <div className="flex items-center justify-center gap-6 py-6">
          <button 
            ...
            className="p-3 rounded-2xl bg-white/5 border border-white/10 text-gray-500 hover:text-cyan-500 hover:border-cyan-500/30 disabled:opacity-10 transition-all cursor-pointer group"
          >
            <ChevronLeft size={24} className="group-hover:-translate-x-1 transition-transform" />
          </button>
          <div className="flex flex-col items-center">
             <span className="text-[10px] font-black text-cyan-500 uppercase tracking-[0.5em]">Sector</span>
