...
import { Loader2, CreditCard, Eye, CheckCircle, XCircle, Search, Filter } from "lucide-react";
...
export default function PaymentArchiveController() {
  ...
  const fetchHistory = useCallback(async () => {
    try {
      ...
      if (res.data.success) {
        const archiveOnly = res.data.result.filter(
          (req) => req.status === "approved" || req.status === "rejected"
        );
        setRequests(archiveOnly);
      }
    } catch (error) {
      toast.error("Failed to sync financial archive");
    } finally {
      setLoading(false);
    }
  }, []);
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
        <div className="space-y-4">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-cyan-500/30 bg-cyan-500/5 backdrop-blur-md">
            ...
            <span className="text-[10px] font-black text-cyan-500 uppercase tracking-[0.3em]">
              {filteredRequests.length} Verified_Transactions
            </span>
          </div>
          <h2 className="text-4xl font-black text-white uppercase tracking-tighter">Financial_Archive</h2>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto">
          ...
          <div className="relative w-full lg:w-48 group">
            ...
            <select
              className="w-full bg-white/5 border border-white/10 backdrop-blur-xl rounded-2xl py-4 pl-12 pr-4 text-[10px] font-bold text-white uppercase tracking-widest focus:outline-none focus:border-cyan-500/40 transition-all appearance-none cursor-pointer"
              onChange={(e) => setStatusFilter(e.target.value)}
              value={statusFilter}
            >
              <option value="all" className="bg-[#050505]">ALL_STATUS</option>
      ...
      <div className="rounded-[2.5rem] border border-white/5 bg-white/[0.02] backdrop-blur-md overflow-hidden shadow-2xl">
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead>
              <tr className="border-b border-white/5 bg-white/[0.03]">
                ...
                <th className="p-7 text-[9px] font-black text-gray-500 uppercase tracking-widest">Sync_State</th>
              ...
              {loading ? (
                ...
                <tr key={req._id} className="hover:bg-white/[0.05] transition-all group">
                  ...
                  <td className="p-7">
                    ...
                    <p className="text-[9px] text-gray-500 mt-1 font-medium italic">{req.userEmail}</p>
                  </td>
                  <td className="p-7">
                  ...
                  <td className="p-7 text-center">
                    <Link
                      ...
                      className="inline-flex p-4 bg-white/5 border border-white/10 hover:border-cyan-500/50 hover:bg-cyan-500/10 text-gray-400 hover:text-cyan-500 rounded-2xl transition-all active:scale-90"
                    >
                      <Eye size={18} />
                    </Link>
                  </td>
        ...
        {!loading && filteredRequests.length === 0 && (
          <div className="p-32 text-center flex flex-col items-center gap-4">
             <CreditCard className="text-gray-800" size={48} />
             <p className="text-[10px] font-black text-gray-600 uppercase tracking-[0.4em]">No archived transactions found</p>
          </div>
        )}
      </div>
    </div>
