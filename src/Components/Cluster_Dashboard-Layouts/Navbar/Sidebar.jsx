...
const Sidebar = () => {
  ...
  const userLinks = [
    ...
    },
    {
      ...
      path: "/my-cluster/dashboard/urgent-payments",
    },
  ];
  ...
  const adminLinks = [
    {
      name: "Manage Users",
      icon: <Users size={20} />,
      path: "/my-cluster/dashboard/admin/users",
    },
    {
      name: "Create User",
      icon: <UserPlus size={20} />,
      path: "/my-cluster/auth/register",
    },
    {
      name: "Node Control",
      ...
      children: [
        {
          name: "All Orders",
          path: "/my-cluster/dashboard/admin/orders/all-orders",
        },
        {
          name: "Active Nodes",
      ...
      children: [
        {
          ...
          path: "/my-cluster/dashboard/admin/payments/pending-payments",
        },
        {
          name: "Confirm-Payments",
          path: "/my-cluster/dashboard/admin/payments/confirm-payments",
        },
        {
          name: "Terminated",
          path: "/my-cluster/dashboard/admin/payments/terminated-payments",
        },
        {
          name: "Archive Payments",
          path: "/my-cluster/dashboard/admin/payments/archive",
        },
      ],
    },
  ];
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-[70] w-64 border-r border-cyan-500/10 backdrop-blur-3xl bg-black/40 transition-transform duration-300 lg:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="flex flex-col h-full p-6 text-white relative">
          <button
            onClick={() => setIsOpen(false)}
            className="lg:hidden absolute top-6 right-6 text-cyan-400 cursor-pointer"
          >
            <X size={20} />
          </button>

          <div className="mb-10 px-4">
            <h2 className="text-xl font-black tracking-tighter text-cyan-400 uppercase">
              Choice Tech.
            </h2>
              ...
              <div key={item.name}>
                {item.children ? (
                  <div>
                    ...
                    <div
                      className={`overflow-hidden transition-all duration-300 ${openDropdown === item.name ? "max-h-40 mt-2 ml-4 border-l border-cyan-500/20" : "max-h-0"}`}
                    >
          ...
          <div className="mt-auto border-t border-cyan-500/10 pt-6">
            <button
              onClick={signOutUser}
              className="flex items-center gap-4 px-4 py-3 w-full text-red-400 hover:bg-red-500/10 rounded-xl transition-all cursor-pointer"
            >
              <LogOut size={20} />
