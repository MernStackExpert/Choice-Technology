...
import { 
  Cpu, Activity, Zap, Terminal, 
  Settings, Construction, Loader2, Binary 
} from "lucide-react";
      ...
      >
        ...
        <div className="flex flex-col items-center text-center space-y-10">
          ...
          <div className="w-full max-w-md space-y-4">
            <div className="flex justify-between items-end mb-2">
              <span className="text-[10px] font-black text-cyan-500 uppercase tracking-widest flex items-center gap-2">
                <Loader2 size={12} className="animate-spin" /> Node_Construction: {progress}%
              </span>
              <span className="text-[10px] font-mono text-gray-600">RA_INST_0982</span>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full pt-10">
            <StatusBox icon={<Binary size={16}/>} label="Source_Code" value="Encrypting" />
            <StatusBox icon={<Zap size={16}/>} label="Neural_Core" value="Optimizing" />
            <StatusBox icon={<Activity size={16}/>} label="Grid_Link" value="Pending" />
          </div>
          ...
          <div className="flex items-center gap-4 p-6 bg-black/40 border border-white/5 rounded-[2rem] w-full max-w-lg">
            ...
            <p className="text-[10px] text-gray-500 text-left font-bold uppercase tracking-widest leading-relaxed">
              Choice Technology is architecting a new node. Your latest projects will be synchronized upon <span className="text-cyan-500">Matrix_Completion</span>.
            </p>
          </div>
        </div>
      </motion.div>
...
const StatusBox = ({ icon, label, value }) => (
  <div className="p-6 bg-white/[0.03] border border-white/5 rounded-3xl flex flex-col items-center gap-2 group hover:bg-white/[0.06] transition-all">
    ...
    <p className="text-[10px] font-bold text-white uppercase tracking-tighter">{value}</p>
  </div>
);